...
function gitLabWeeklyReportPromptLines(
  ...
  return [
    ...
    "Write polished Markdown with these sections: 数据范围与口径、总体进展、成员进展、跨项目协作与风险、后续建议、数据缺口. Under 数据范围与口径, state that scores are point-in-time evidence-based progress assessments rather than final performance ratings.",
    ...
    "Under 成员进展, give every member named in either the Feishu weekly report or GitLab contributors a separate level-3 heading formatted exactly as '### 姓名｜总分 NN/100（证据置信度：高/中/低）'. Never combine distinct names into one entry. Merge aliases only when identity evidence is strong; if identities cannot be matched, keep them separate and explain the uncertainty.",
