...
function gitLabWeeklyReportPromptLines(
  ...
  return [
    ...
    "Write polished Markdown with these sections: 数据范围与口径、总体进展、成员进展、跨项目协作与风险、后续建议、数据缺口. Do not create a standalone 周报与代码关联性 section; place every relation conclusion, label count, and evidence list inside the corresponding eligible member's 成员进展 entry. Do not repeat a cross-member relation summary outside the member entries. Under 数据范围与口径, state that scores are point-in-time evidence-based progress assessments rather than final performance ratings.",
    ...
    "Under each eligible member heading, add the exact level-4 heading '#### 周报与代码关联性' immediately before the member's scoring bullets. Keep this subsection scoped to that member only: begin with a compact count of the six relation labels for that member, then list each material weekly-report work item with its project, relation label, confidence, short report claim, concrete GitLab evidence or explicit absence, and conclusion. If the member has GitLab activity but no matching weekly-report item, include the high-signal commits here as 仅代码 and state that no report item was matched. If the member has a confirmed account but no commits, include the verified report items as 仅周报 or 无法判断 with an explicit missing-code-evidence note. Never put another member's work in the subsection and never create a separate top-level relation section. Do not hide a mismatch behind aggregate scores.",
    "After that member-scoped relation subsection, write exactly three information-dense bullets totaling roughly 300-500 Chinese characters: 分项评分 must show all four awarded scores in '目标与交付 NN/40；质量与可信证据 NN/25；协作与影响 NN/20；风险管理与闭环 NN/15' form; 客观依据 must connect each awarded score to specific weekly-report statements and concrete GitLab code, document, configuration, or test changes when available, explicitly state the applicable relation label and confidence, label evidence as 周报、GitLab、or 双源印证, and include up to two useful commit links; 风险与后续 must explain an evidence-backed blocker, discrepancy, unfinished item, or evidence gap and one specific next action. List all involved projects compactly, but do not substitute project names or commit counts for actual progress analysis.",
