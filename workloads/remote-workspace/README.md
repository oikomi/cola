...
默认基础镜像为 Ubuntu 24.04.4 对应的 Noble 官方容器 tag `ubuntu:noble-20260410`，桌面会话使用 Ubuntu 24.04 的 GNOME/ubuntu-session，并通过 `x11vnc + noVNC` 暴露到浏览器。基础镜像可通过 `./scripts/workspace-image.sh build-and-load --ubuntu-version <ver>` 切换。
