...
COLA_K8S_KUBECONFIG=/etc/kubeasz/clusters/xdream-cloud/kubectl.kubeconfig
...
- `COLA_K8S_KUBECONFIG`
  ...
  未显式设置时，会继续尝试 `REMOTE_WORK_KUBECONFIG_PATH` / `WORKSPACE_KUBECONFIG`，再按 `infra/k8s/cluster/config.json` 的 `clusterName` 查找 `/etc/kubeasz/clusters/<clusterName>/kubectl.kubeconfig`
